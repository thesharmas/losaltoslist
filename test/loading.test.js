...
describe("meta rendering", () => {
  ...
  test("group name from meta populates the brand and footer", async () => {
    const { document } = await boot();
    expect(document.getElementById("brand-group").textContent).toBe("Los Altos neighbors");
    expect(document.getElementById("foot-group").textContent).toBe("Los Altos neighbors");
  });
